...
class _AddDoctorScreenState extends State<AddDoctorScreen> {
  ...
  final List<String> _specialties = ['Tai mũi họng', 'Nội thần kinh', 'Mắt', 'Nha khoa', 'Chấn thương chỉnh hình'];
  List<WorkExperience> _workExperiences = [WorkExperience()]; // Default value with an empty WorkExperience
  ...
  List<DropdownMenuItem<String>> _generateYearItems() {
    final List<String> years = List.generate(30, (index) => (2023 - index).toString());
    return years.map<DropdownMenuItem<String>>(
   ...
   void _addTimelineDescription(int index) {
    ...
    _experienceController.text = '${ _experienceController.text}\n$timelineDescription';
  }
  ...
  try {
    ...
    if (name.isEmpty || email.isEmpty || description.isEmpty || experience.isEmpty || _selectedImage == null) {
    ...
    DocumentReference doctorRef = await firestore.collection('users').doc(uid);
      String? downloadURL;
